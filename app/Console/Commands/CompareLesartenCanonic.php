...
class CompareLesartenCanonic extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'compare:lesarten:canon';
    ...
    public function handle()
    {
        ...
        $output = $outputDir . "leser_similarities_canon_" .Carbon::now("Europe/Berlin")->now()->format("Ymd_his") . ".csv";
        ...
        foreach(Koranstelle::getAllWordsExceptHeaders() as $koranstelle)
        {
            ...
            foreach($lesearten as $leseart)
            {
                ...
                foreach($leseart->leser as $leser)
                {
                    ...
                    if (array_key_exists($leserId, $leserKanonischIds))
                    {
